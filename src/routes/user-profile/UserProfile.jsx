...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    if (isError) {
      message.error(error?.userData?.message)
    }
  ...
  return (
      <div>
        <Header/>
        <div className="w-full flex justify-center items-center flex-col mx-auto gap-4 pt-[100px] mb-[80px]">
          <div>
            <div className="max-w-[200px] border rounded-full p-2 my-5">
              {
                data?.payload?.avatar ?
                    <Image className="rounded-full" src={data?.payload?.avatar} alt="User avatar"/> :
